...
export default function CapturePage() {
  ...
  useEffect(() => {
    searchPatients("").then(setAllPatients).catch(() => {});
  }, []);
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 space-y-4">
          ...
          <select
            ...
            className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-200 bg-white"
          >
            {providers.map((p) => (
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 space-y-4">
          ...
          {selectedPatient ? (
            <div className="flex items-center justify-between rounded-lg px-4 py-3" style={{ background: "var(--brand-primary-light)" }}>
              <div>
            ...
            <div ref={patientSearchRef} className="relative">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-lg pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-200"
                />
              </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-200 bg-white"
              >
                {VISIT_TYPES.map((v) => (
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 space-y-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{
                background: !audioFile && !recordedBlob ? "var(--brand-primary)" : "white",
                color: !audioFile && !recordedBlob ? "white" : "#6B7280",
                borderColor: !audioFile && !recordedBlob ? "var(--brand-primary)" : "#E5E7EB",
              }}
            >
              <Mic size={12} /> Record
            </button>
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors"
              style={{
                background: audioFile ? "var(--brand-primary)" : "white",
                color: audioFile ? "white" : "#6B7280",
                borderColor: audioFile ? "var(--brand-primary)" : "#E5E7EB",
              }}
            >
              ...
              {!recordedBlob ? (
                <div className="flex items-center gap-4">
                  <button
                    ...
                    style={{ background: isRecording ? "#EF4444" : "var(--brand-primary)" }}
                  >
                ...
                <div className="flex items-center gap-3 rounded-lg px-4 py-3" style={{ background: "var(--brand-primary-light)" }}>
                  <FileAudio size={20} style={{ color: "var(--brand-primary)" }} />
                  <div className="flex-1">
                    <div className="text-sm font-medium text-gray-800">
                      Recording ({formatTime(recordingTime)})
                    </div>
                    <div className="text-xs text-gray-500">{(recordedBlob.size / 1024).toFixed(0)} KB</div>
                  </div>
                  <button type="button" onClick={() => { setRecordedBlob(null); setRecordingTime(0); }} className="text-gray-400 hover:text-gray-600">
                    <X size={16} />
                  </button>
                </div>
              )}
            ...
            <div>
              ...
              {audioFile ? (
                <div className="flex items-center gap-3 rounded-lg px-4 py-3" style={{ background: "var(--brand-primary-light)" }}>
                  ...
                  <button type="button" onClick={() => setAudioFile(null)} className="text-gray-400 hover:text-gray-600">
                    <X size={16} />
                  </button>
                </div>
              ) : (
                <div
                  className="border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors hover:border-teal-300"
                  style={{ borderColor: "#CBD5E1" }}
                  onDragOver={(e) => e.preventDefault()}
                  onDrop={(e) => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { setAudioFile(f); setRecordedBlob(null); } }}
                  onClick={() => fileInputRef.current?.click()}
                >
                  <Upload size={20} className="mx-auto text-gray-300 mb-1" />
                  <div className="text-xs text-gray-500">
                    Drop audio file or <span style={{ color: "var(--brand-primary)" }}>browse</span>
                  </div>
          ...
          <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 space-y-3">
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${progress}%`,
                  background: status === "error" ? "#EF4444" : status === "complete" ? "var(--brand-primary)" : "var(--brand-accent)",
                }}
              />
            </div>
            <div className="flex items-center gap-2 text-sm">
              {(status === "processing" || status === "creating" || status === "uploading") && <Loader2 size={14} className="animate-spin text-teal-500" />}
              {status === "complete" && <CheckCircle2 size={14} style={{ color: "var(--brand-primary)" }} />}
            ...
            {encounterId && (
              <div className="text-xs text-gray-400">Encounter: {encounterId}</div>
            )}
        ...
        <p className="text-xs text-gray-400">
          Audio data is processed securely on-premise. Zero PHI egress.
        </p>
      </form>
    </div>
  );
